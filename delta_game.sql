...
CREATE TABLE `order` (
  ...
  `player_id2` bigint DEFAULT NULL COMMENT '辅助打手ID(双人指派)',
  ...
  KEY `idx_player_id2_status` (`player_id2`,`status`) USING BTREE,
  KEY `idx_status` (`status`) USING BTREE,
...
CREATE TABLE `user` (
  ...
  `total_points` int NOT NULL DEFAULT '0' COMMENT '历史累计积分(用于等级)',
  `level_code` varchar(32) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL DEFAULT 'BRONZE' COMMENT '会员等级代码',
  `level_name` varchar(64) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL DEFAULT '青铜伴星' COMMENT '会员等级名称',
  `created_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
  PRIMARY KEY (`id`) USING BTREE,
  UNIQUE KEY `uk_openid` (`openid`) USING BTREE
...
CREATE TABLE `points_detail` (
  ...
  `balance` int NOT NULL DEFAULT '0' COMMENT '变动后当前积分余额',
  `type` varchar(32) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL DEFAULT '' COMMENT '类型: ORDER_CONSUME/ADMIN_ADD/ADMIN_DEDUCT',
  ...
  KEY `idx_user_id` (`user_id`) USING BTREE,
  KEY `idx_order_id` (`order_id`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci ROW_FORMAT=DYNAMIC COMMENT='用户积分明细';
/*!40101 SET character_set_client = @saved_cs_client */;
